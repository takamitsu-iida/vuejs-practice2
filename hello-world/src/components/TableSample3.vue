    ...
    <div class="form-container">
      ...
      <div class="columns">
        <div class="column is-12">
          <div class="field">
            ...
            <div class="control">
              <label class="checkbox" v-for="item in area_list" v-bind:key="item.id">
                <input type="checkbox" v-bind:value="item.value" v-model="checked_areas">
                  {{ item.value }}
              </label>
            </div>
          </div>
          <span v-if="DEBUG">Selected: {{ checked_areas }}</span>
      ...
      <div class="columns">
        <div class="column is-4">
          <div class="field">
            <label class="label">機種</label>
            <div class="control">
              <label class="checkbox" v-for="item in device_list" v-bind:key="item.id">
                <input type="checkbox" v-bind:value="item.value" v-model="checked_devices">
                  {{ item.value }}
              </label>
            </div>
          </div>
          <span v-if="DEBUG">Selected: {{ checked_devices }}</span>
        </div>

        <div class="column is-4">
          <div class="field">
            <label class="label">機器検索</label>
            <div class="control">
              <input class="input" type="text" placeholder="Text input" v-model.trim="nodename">
            </div>
          </div>
          <span v-if="DEBUG">Selected: {{ nodename }}</span>
        </div>

      </div>

      <span>{{ filtered_row.length }}</span>
...
function init_data() {
  ...
  const devices = ["si-r", "sr-s", "sr-x", "ios"];

  const area_list = [];
  for (let i = 0; i < areas.length; i++) {
    area_list.push({ id: i, value: areas[i] });
  }

  const device_list = [];
  for (let i = 0; i < devices.length; i++) {
    device_list.push({ id: i, value: devices[i] });
  }
...
export default {
  ...
  computed: {
    filtered_row: function() {
      // bind(this)をしたくないのでアロー関数でthisを拘束する
      let filtered = this.rows.filter(node => {
        return this.checked_areas.indexOf(node.area) >= 0;
      });
      return filtered;
    }
  }
...
.table-container {
  ...
  width: auto;
  /*height: 500px;*/
  height: calc(100vh - 200px); /* 経験則的に200px */
}
